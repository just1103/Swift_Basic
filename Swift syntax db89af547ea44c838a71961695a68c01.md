...
Last Edited Time: July 5, 2021 9:12 PM
Type: 언어
...
- 디이니셜라이저 deinit
    - 클래스에만 구현 가능합니다.
    - 클래스 인스턴트는 참조 타입이므로 더 이상 참조가 필요 없을 때 메모리에서 해제됨 (인스턴스의 소멸)
    - 클래스 인스턴스의 소멸 직전에 호출됩니다. 자동으로 호출되므로 직접 호출할 수 없습니다.
...
2. 17. 타입 캐스팅 (타입 확인-is, 업캐스팅-as, 다운캐스팅-as?, as!)
    - L/G
...
7. 22. Higher-order Function (고차 함수)
    - 특징
